...
import { detectGPU, formatBytes } from './utils';
...
export class Optimizer {
  ...
  async optimize(model) {
    ...
    console.log(`[Optimizer] Done in ${(performance.now() - start).toFixed(0)}ms`);
  }
  ...
  optimizeTextures(model) {
    ...
    const effectiveMax = Math.min(DEFAULT_CONFIG.maxTextureSize, gpuMax);
    let optimized = 0;
    ...
    model.traverse((child) => {
      ...
      for (const mat of mats) {
        ...
        for (const key of keys) {
          ...
          tex.anisotropy = gpu.tier === 'high' ? 16 : gpu.tier === 'mid' ? 8 : 4;
